...
void Drive::CopyFileToLocalDisk(File *Dest, Progress *out)
{

}

void Drive::CopyFileToLocalDisk(string Path, Progress *out)
{
...
vector<Drive> Drive::GetFATXDrives( bool HardDisks )
{
    ...
    if (HardDisks)
    {
        ...
        for (int i = 0; i < (int)Disks.size(); i++)
        {
            ...
            Streams::xDeviceStream* DS = new Streams::xDeviceStream(Disks.at(i).Path);
            if (DS->Length() == 0 || DS->Length() < HddOffsets::Data)
            {
                continue;
            }
...
if (dir != NULL)
{
    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        if (exp.exactMatch(ent->d_name))
        {
            ...
            wcscpy(curdir.Path, L"/dev/");
            TCHAR temp[0x50] = {0};
            mbstowcs(temp, ent->d_name, ent->d_namlen);
            wcscpy(&(curdir.Path[0]) + 5, temp);
            //swprintf(curdir.Path, (ent->d_namlen * 2) + 1, L"/dev/r%hs", &(ent->d_name[0]) + 5);

            int device;
            if ((device = open(ent->d_name, O_RDONLY, 0)) > 0)
            {
#ifdef __linux
                hd_driveid hd;
